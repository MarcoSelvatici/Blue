...
let generalTests = [
    ...
    "func def/ app",
    Ok (FuncDefExp {FuncName = "f"; FuncBody =  LambdaExp { LambdaParam = "x" ; LambdaBody = FuncApp( FuncApp( BuiltInFunc Plus, Identifier "x"), Literal (IntLit 2)) }; Rest = FuncApp( Identifier "f", Literal (IntLit 5));}),
    Ok (Literal (IntLit 7));
...
let generalErrorTests = [
    ...
    Error <| TypeCheckerError "This is an error message!",
    Error (TypeCheckerError "This is an error message!");
